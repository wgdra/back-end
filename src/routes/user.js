...
router.post('/login', async (request, response) => {
  try {
    ...
    if(dataRequest.username == "" || dataRequest.password == "") {
      ...
      return response.status(400).json({status: false, data: "", msg:"Bạn cần nhập đủ cả tài khoản và mật khẩu"})
    } 
    if(dataRequest.username != "" && dataRequest.password != ""){
      const { data, error } = await request.supabase
        .from('user')
        ...
        if( data.password != encrypt(dataRequest.password)) {
          ...
          const token = jwt.sign({ id, username, role }, process.env.DATABASE_KEY_SECRET, { expiresIn: '1h' });
          // const refreshToken = jwt.sign(user, process.env.REFRESH_SECRET_KEY, { expiresIn: '3h' });
          return response.status(200).json({status: true, data: {id, username, role, token}, msg:"Đăng nhập thành công"})
        }
    }
  ...
  try {
    ...
    if(checkToken.status == true) {
      if(checkToken.data.role == tokenControl.ROLE.Admin) {
        ...
        if(checkData == null) {
          const { data: insertData, error: insertError } = await request.supabase.from('user').insert({
